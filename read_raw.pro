...
pro read_raw
print,systime()
  dir='G:\基于高光谱遥感影像的化学态势项目\白城数据\2022_11_08_15_43_00\'
  ...
  ;files[0]=dir+'20220127152049116_0053_LWIR01.dat'

  dir1='G:\基于高光谱遥感影像的化学态势项目\白城数据\2022_11_08_15_43_00\处理\'
  ...
  type_code=make_array(20,/int)
  type_code[12]=2

  frame=mk_array([ns,nb],12)
  ;frame1=mk_array(ns*,12)
  line=mk_array(ns,12)
  pixel=0u
  line_num0=-1L
  p=long64(0)
  ...
  for f=0,nf-1 do begin
    ...
    ;分块处理，分块设置
    finfo=file_info(files[f])
    np=finfo.size/2
    print,np/328
    seg=long64(5000)*long64(ns)*long64(nb)
    nseg=np/seg+1
    seg_pos=make_array(nseg,/l64)
    for s=0,nseg-1 do begin
      seg_pos[s]=s*seg gt np-1 ? np-1:s*seg
    endfor

    data=mk_array([seg],12)
    data1=mk_array([np-seg_pos[nseg-1]],12)
    ...
    file_out=dir1+file_basename(files[f],'.dat')+'_tmp.tmp'
    print,strtrim(string(f),2),'  ',file_out
    openw,lun_out,file_out,/get_lun
    ii=0
    for s=0,nseg-1 do begin
      if s eq 0 and nseg gt 1 then begin
        point_lun,lun,seg_pos[s]*2
        readu,lun,data
        index=where(data eq 65021,count,/L64)
        di=make_array(count-1,/int)
        for i=0,count-2 do begin
          di[i]=index[i+1]-index[i]
        endfor
        
        index2=where(di eq 328,count2,/L64)
        for i=0,count2-1 do begin
          line[*]=data[index[index2[i]]:index[index2[i]]+327]
          if line[0] eq  65021 and line[ns-1] eq  64507 then begin
            writeu,lun_out,line
            line[*]=0
            nll=nll+1
            if nll mod 10000 eq 0 then print,nll
          endif
        endfor

        if seg-index[count-1] lt ns then begin
          line[0:seg-1-index[count-1]]=data[index[count-1]:seg-1]
        endif
        if line[0] eq  65021 and line[ns-1] eq  64507 then begin
          writeu,lun_out,line
          line[*]=0
      ...
      endif else if s gt 0 and s lt nseg-1 then begin
        point_lun,lun,seg_pos[s]*2
        readu,lun,data
        index=where(data eq 65021,count,/L64)
        
        index1=where(line eq 0,count1)
        if count1 gt 0 and count1 lt ns and index[0]-1 gt 0 then begin
          if index1[0]+index[0]-1 lt ns then begin
            line[index1[0]:index1[0]+index[0]-1]=data[0:index[0]-1]
          endif else begin
            line[index1[0]:327]=data[0:327-index1[0]]
          endelse
          if line[0] eq  65021 and line[ns-1] eq  64507 then begin
            writeu,lun_out,line
            line[*]=0
        ...
        point_lun,lun,seg_pos[s]*2
        readu,lun,data1
        index=where(data1 eq 65021,count,/L64)
        di=make_array(count-1,/int)
        for i=0,count-2 do begin
          di[i]=index[i+1]-index[i]
        endfor
        ...
        if count1 gt 0 and count1 lt ns and index[0]-1 gt 0 then begin
          if index1[0]+index[0]-1 lt ns then begin
            line[index1[0]:index1[0]+index[0]-1]=data1[0:index[0]-1]
          endif else begin
            line[index1[0]:327]=data1[0:327-index1[0]]
          endelse
          if line[0] eq  65021 and line[ns-1] eq  64507 then begin
            writeu,lun_out,line
            nll=nll+1
        ...
        index2=where(di eq 328,count2,/L64)
        for i=0,count2-1 do begin
          line[*]=data1[index[index2[i]]:index[index2[i]]+327]
          if line[0] eq  65021 and line[ns-1] eq  64507 then begin
            writeu,lun_out,line
            line[*]=0
            nll=nll+1
      ...
      if np-seg_pos[nseg-1]-index[count-1] eq ns then begin
        line[*]=data1[index[count-1]:np-seg_pos[nseg-1]-1]
      endif
      if line[0] eq  65021 and line[ns-1] eq  64507 then begin
        writeu,lun_out,line
        line[*]=0
    ...
    free_lun,lun,lun_out
    
    openr,lun,file_out,/get_lun
    
    file_out1=dir1+file_basename(files[f],'.dat')+'_RAW.dat'
    print,strtrim(string(f),2),'  ',file_out1
    openw,lun_out1,file_out1,/get_lun
    
    readu,lun,line
    while ~eof(lun) do begin
      if line[6] eq 0 then begin
        frame[*,0]=line
        ii=0
        repeat begin
          ii=ii+1
          readu,lun,line
          frame[*,ii]=line
        endrep until (line[6] eq 0 or ii eq 256 or eof(lun))

        if frame[6,0] eq 0 and frame[6,nb-1] eq 256 then begin
          nl=nl+1
          if nl mod 1000 eq 0 then print,nl
;          if nl eq 686 then begin
;            tvscl,frame
;            print,frame[*,253]
;          endif
          writeu,lun_out1,frame[7:326,1:256]
          frame[*,*]=0
        endif
      endif else begin
        readu,lun,line
    ...
    free_lun,lun,lun_out1
    
    hdr_file_setup,file_out1,$
          ns=320,$
          nl=nl,$
          nb=256,$
          data_type=12,$
